...
class TestAssess:
    ...
    def test_unencoded_program_state_warns_and_is_incomplete(self):
        c = coverage.assess("WA")
        assert c["encoded"] is False
        assert c["complete"] is False
        assert c["program"] == "Washington Paid Family and Medical Leave"
        assert any("not yet encoded" in w for w in c["warnings"])
        assert any("Washington Paid Family and Medical Leave" in w for w in c["warnings"])
    ...
    def test_lowercase_input_normalizes(self):
        assert coverage.assess("wa")["complete"] is False
...
class TestDeterminationCoverage:
    def test_washington_determination_is_flagged_incomplete(self):
        result = determine(facts_for("WA"))
