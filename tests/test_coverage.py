...
class TestAssess:
    ...
    def test_unencoded_program_state_warns_and_is_incomplete(self):
        c = coverage.assess("HI")
        assert c["encoded"] is False
        assert c["complete"] is False
        assert c["program"] == "Hawaii Temporary Disability Insurance"
        assert any("not yet encoded" in w for w in c["warnings"])
        assert any("Hawaii Temporary Disability Insurance" in w for w in c["warnings"])
    ...
    def test_lowercase_input_normalizes(self):
        assert coverage.assess("hi")["complete"] is False
