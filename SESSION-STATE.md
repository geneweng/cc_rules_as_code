...
*Handoff note so work can resume after a restart. Last updated after commit `7eb28ba`.*
...
## Current state (all committed, tree clean, 94 tests passing)

**The engine** (`openleave/`) — eight regimes across six states:
- Federal FMLA; CA (CFRA + PFL); MA (PFML); MN (Paid Leave); NJ (FLI); NY (PFL); WA (PFML).
...
- **The disclaimer is load-bearing.** Every surface says statutory parameters are
  "approximations pending counsel review." The WA/MA/NJ (and all) figures came from web
  research (2026 rates) and have NOT been verified by an employment lawyer. This must happen
  before anyone relies on a determination. This is the #1 gate before real use.
...
- **Traction is internal only** — 94 tests and a working pipeline are engineering traction, not
...
1. **More jurisdictions.** Declared gaps still warned about: CO, CT, DC, DE, MD, ME, OR, RI.
